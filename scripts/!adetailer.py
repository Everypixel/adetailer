...
class AfterDetailerScript(scripts.Script):
    ...
    def _postprocess_image_inner(
        ...
        for j in range(steps):
            ...
            if args.ad_use_custom_color_equalization:
                ...
                face_original_mask = self.process_face_parser(face_original)
                face_enhanced_mask = self.process_face_parser(face_enhanced)

                face_original_mask, face_original_mask_blurred = self.get_blurred_mask_by_face_parser_mask(face_original_mask)
                face_enhanced_mask, face_enhanced_mask_blurred = self.get_blurred_mask_by_face_parser_mask(face_enhanced_mask)

                matched_img = self.PT.pdf_transfer(img_arr_in=face_enhanced*face_original_mask[..., np.newaxis], img_arr_ref=face_original*face_original_mask[..., np.newaxis], regrain=True)
                matched_img = face_enhanced*(1-face_original_mask[..., np.newaxis]) + matched_img*face_original_mask[..., np.newaxis]
    ...
    def process_face_parser(self, face_crop):
        face_crop = Image.fromarray(face_crop)

        inputs = self.face_parser__image_processor(images=face_crop, return_tensors="pt")
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        outputs = self.face_parser(**inputs)
    ...
    def get_blurred_mask_by_face_parser_mask(self, mask, kernel_erode=2, kernel_blur=20):
